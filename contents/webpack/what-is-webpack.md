...
웹팩에 대해 들어본 적은 있지만 제대로 공부한 적은 없어서 이번에 개념부터 차근차근 공부했습니다. 웹팩이란 무엇인지, 어떠한 장점이 있는지 전반적인 개념에 대해 알아보고 웹팩 구성 요소 중 **로더**에 대해 상세하게 정리했습니다.
...
> **번들러**란 필요한 의존성을 추적하여 해당하는 의존성들을 그룹핑 해주는 도구입니다.
...
> entry 속성은 웹팩에서 **웹 자원을 변환하는 데 필요한 진입점이자 자바스크립트 파일 경로**입니다.
...
번들된 결과물을 처리할 위치를 output 에 기록합니다.
...
> 웹팩의 기본적인 동작에 **추가적인 기능을 제공하는 속성**입니다.
...
> 웹팩은 JS 파일과 IMG 파일 등 모두 **모듈로 인식**합니다.

그래서 JS 코드에서 import 구문을 사용해서 IMG 와 CSS 파일을 가져올 수 있는 것입니다. 모듈을 지원하지 않는 브라우저에서도 웹팩을 사용하면 모듈을 적용할 수 있습니다.

![webpack-2](./images/what-is-webpack/webpack-2.jpeg)

위에서 본 사진을 기준으로 **왼쪽 위 .js 파일**을 **entry** 라고 합니다. entry 를 기준으로 모든 모듈을 찾은 후 하나의 코드로 만들어 줍니다. 따라서 오른쪽 그림을 보면 하나의 .js 파일이 생성된 것을 볼 수 있습니다. 이렇게 만들어진 **결과물**을 **output** 이라고 합니다.
...
> mode 옵션은 웹팩에 내장된 **최적화 방법 중 어느 것을 사용할지 웹팩에게 알려주는 역할**을 합니다.
...
> entry 옵션은 **애플리케이션 번들링을 시작할 지점을 의미**합니다. 만약 배열을 값으로 전달하면, 배열에 담긴 모든 요소가 번들링됩니다.

HTML 페이지마다 하나의 포인트를 가집니다. 그래서 SPA(Single Page Application)는 하나의 포인트를 가지게 되고, MPA(Multiple Page Application)는 페이지별로 하나의 포인트를 가지게 됩니다.
    ...
    rules: [
      {
        ...
        use: ['css-loader'], // css-loader 를 적용
      },
    ],
  },
};
...
module.exports = {
  ...
  module: {
    rules: [
      {
        test: /\.css$/,
        use: ['style-loader', 'css-loader'], // style-loader 를 앞에 추가
      },
    ],
  },
};
